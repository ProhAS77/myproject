    ...
    def handle(self, *args, **kwargs):
        ...
        for i in range(1, count + 1):
            products.append(Product(
                ...
                description='длинное описание продкута, которое и так никто не читает',
                price=uniform(0.01, 999_999.99),
                quantity=randint(1, 10_000),
                rating=uniform(0.01, 9.99),
            ))
        Product.objects.bulk_create(products)#bulk_create - принимает список объектов и через один insert запрос добавляетяс в БД
